...
export const StudentProfile: React.FC<ProfileProps> = ({ studentId, onBack }) => {
  ...
  // Connect to actual master syllabus from constants.ts
  ...
  // Always derive current topic from the MASTER syllabus to ensure "Connection"
    ...
    <div className="space-y-8 animate-in slide-in-from-right-8 duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
            <div className="flex flex-col items-center text-center mb-8">
              <div className="w-24 h-24 rounded-[2rem] bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-4xl font-black text-white shadow-xl shadow-amber-500/20 mb-4 transform hover:scale-105 transition-transform cursor-default">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className={`h-2.5 w-full rounded-full overflow-hidden border ${student.billing.feeStatus === 'due' ? 'bg-white border-slate-100' : 'bg-white/5 border-white/10'}`}>
                  <div 
                    className={`h-full transition-all duration-1000 ${student.billing.feeStatus === 'due' ? 'bg-red-500' : 'bg-amber-500'}`}
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* Academic Progression - Now connected to actual Master Syllabus */}
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div className="mt-10 pt-8 border-t border-slate-50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Master Syllabus v2.5 Verified</p>
              </div>
              <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest bg-amber-50 px-3 py-1.5 rounded-xl">
                {Math.round((student.currentTopicIndex / (masterSyllabus.length || 1)) * 100)}% Complete
              </p>
            </div>
          </div>
        </div>
